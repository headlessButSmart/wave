...
export const
  ...
  connect = (server:S, path: S, route:S, producer:S, data, handle: WaveEventHandler): Wave => {
    let
    ...
    const
      ...
      hash = producer?producer:window.location.hash,
      reconnect = (address: S) => {
        ...
        socket.onopen = () => {
          ...
          //Send initial request, if there is a query
          if(data) socket.send(`@ ${slug} ${JSON.stringify({data:data})}`)
        }
        socket.onclose = () => {
